  ...
  return (
    <>
      <a
        ...
        className="sr-only focus:not-sr-only focus:inline-flex focus:mb-3 focus:px-3 focus:py-2 focus:rounded focus:bg-primary focus:text-primary-foreground"
      >
        Skip to business results
      </a>
      <div className="bg-card border border-border rounded-xl p-4 md:p-5 mb-8 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          <div className="md:col-span-5">
            <label htmlFor="business-search" className="block text-sm font-medium mb-1.5">
              Search {industryName}
            </label>
          ...
          {categories.length > 1 && (
            <div className="md:col-span-3">
              <label htmlFor="business-category" className="block text-sm font-medium mb-1.5">
                Provider category
              </label>
              <Select value={category} onValueChange={setCategory}>
                <SelectTrigger id="business-category" aria-label={`Filter ${industryName} by provider category`}>
          ...
          <div className={categories.length > 1 ? "md:col-span-2" : "md:col-span-3"}>
            <label htmlFor="minimum-rating" className="block text-sm font-medium mb-1.5">
              Minimum rating
            </label>
            <Select value={minRating} onValueChange={setMinRating}>
              <SelectTrigger id="minimum-rating" aria-label={`Filter ${industryName} by minimum source-observed rating`}>
          ...
          <div className={categories.length > 1 ? "md:col-span-2" : "md:col-span-4"}>
            <label htmlFor="business-sort" className="block text-sm font-medium mb-1.5">
              Sort results
            </label>
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger id="business-sort" aria-label={`Sort ${industryName} results`}>
      ...
      {filtered.length === 0 ? (
        ...
        <div id="business-results" tabIndex={-1} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 scroll-mt-24">
          {filtered.map((b, i) => (
            <BusinessCard
              key={i}
              business={b}
