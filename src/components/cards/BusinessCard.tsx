...
export const BusinessCard = ({ business, citySlug, industrySlug, qualificationLabel }: Props) => {
  ...
  return (
    <article className="bg-white border rounded-lg p-6 hover:shadow-lg hover:border-[var(--c-sage)] transition-all duration-300 flex flex-col group" style={{ borderColor: 'var(--c-border)' }}>
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {isTopRated && (
              <span className="flex-shrink-0 inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full" style={{ backgroundColor: 'var(--c-copper-lt)', color: 'var(--c-pine)' }}>
                <BadgeCheck className="w-3 h-3" aria-hidden="true" /> 4.8+ source rating
              </span>
            )}
          </div>
          {business.category && (
      ...
      {qualificationLabel && (
        <p className="text-xs leading-relaxed mb-3" style={{ color: 'var(--c-sage)' }}>
          {qualificationLabel}
        </p>
      )}
      ...
      <div className="flex flex-wrap gap-2 pt-4 border-t" style={{ borderColor: 'var(--c-border)' }}>
        ...
        {!business.website && business.websiteStatus === "unconfirmed" && (
          <span className="inline-flex items-center min-h-[44px] px-4 py-2 text-sm border rounded" style={{ borderColor: 'var(--c-border)', color: 'var(--c-sage)' }}>
